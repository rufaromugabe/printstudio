...
import { OCRSuggestion, prepareProductionExport, ProductionMethod, ProductionResult } from "@/lib/production-export";
...
export default function Home() {
  ...
  // Direct is the safe default: it traces the exact uploaded alpha bounds.
  // Advanced cleanup is opt-in because it may intentionally remove background/detail.
  const [advancedVectorize,setAdvancedVectorize]=useState(false);
  ...
  const embroideryRequest=async():Promise<EmbroideryRequest>=>{
    ...
    const request:EmbroideryRequest={
      name:design.name,
      fabricClass:embroideryDefaults.fabricClass,
      regions:result.regions,
      machine:{id:"generic-130x180",name:"Generic 130 x 180 mm",hoopWidthMm:130,hoopHeightMm:180,maxStitches:100000,maxColors:16,minStitchMm:.4,maxStitchMm:12.1,maxJumpMm:12.1},
      printWidthMm:currentView.physicalWidthMm,
      printHeightMm:currentView.physicalHeightMm,
    };
  ...
  const embroideryStitchFieldMM=(()=>{
    const plan=embroidery?.document.plan??[];
    if(!plan.length)return "";
    const boxes=plan.map(b=>b.bounds).filter(b=>b&&Number.isFinite(b.minX)&&Number.isFinite(b.maxX)&&Number.isFinite(b.minY)&&Number.isFinite(b.maxY));
    if(!boxes.length)return "";
    const minX=Math.min(...boxes.map(b=>b!.minX)),minY=Math.min(...boxes.map(b=>b!.minY)),maxX=Math.max(...boxes.map(b=>b!.maxX)),maxY=Math.max(...boxes.map(b=>b!.maxY));
    return `${(maxX-minX).toFixed(1)} × ${(maxY-minY).toFixed(1)} mm stitch field · `;
  })();
  ...
  const rebuildRecognizedText=(suggestion:OCRSuggestion)=>{
    if(!suggestion.recommended)return;
    commit(d=>{
      let changed=false;
      const elements=Object.fromEntries(Object.entries(d.elements).map(([side,layers])=>[side,layers.map(element=>{
        if(element.id!==suggestion.elementId||element.type!=="image")return element;
        changed=true;
        return{...element,type:"text" as const,value:suggestion.text,assetId:undefined,sourceWidth:undefined,sourceHeight:undefined,color:element.color||"#222222",fontSize:Math.max(10,Math.min(element.h*.62,72)),fontFamily:suggestion.fontFamily,fontWeight:suggestion.fontWeight,fontStyle:"normal" as const,textDecoration:"none" as const,textAlign:"center" as const,letterSpacing:0,lineHeight:1.1,strokeColor:"#ffffff",strokeWidth:0,shadow:false,curveType:"straight" as const};
      })]));
      return changed?{...d,elements}:d;
    });
    setSelected(suggestion.elementId);
    closeProduction();
  };
...
type ProductionDialogProps={production:ProductionResult|null;state:"idle"|"preparing"|"error";error:string;method:string;iccCombo:string;setIccCombo:(value:string)=>void;iccCombinations:{id:string;label:string;sourceProfile:string;destinationProfile:string}[];mirrorVinyl:boolean;setMirrorVinyl:(value:boolean)=>void;vinylMaterialClass:VinylMaterialClass;prepareProduction:(mirror?:boolean)=>Promise<void>;close:()=>void;download:()=>void;createAlternate:(format:"pdf"|"tiff"|"zip"|"gang")=>Promise<void>;createAdvanced:(kind:"underbase"|"halftone"|"halftone-fm"|"cmyk")=>Promise<void>;rebuildRecognizedText:(suggestion:OCRSuggestion)=>void;proofState:"none"|"pending"|"approved";formatState:string;gang:{copies:number;width:number;height:number;fillSheet:boolean;gap:number;setCopies:(value:number)=>void;setWidth:(value:number)=>void;setHeight:(value:number)=>void;setFillSheet:(value:boolean)=>void;setGap:(value:number)=>void};history:ExportRecord[];redownload:(record:ExportRecord)=>Promise<void>};
function ProductionDialog({production,state,error,method,iccCombo,setIccCombo,iccCombinations,mirrorVinyl,setMirrorVinyl,vinylMaterialClass,prepareProduction,close,download,createAlternate,createAdvanced,rebuildRecognizedText,proofState,formatState,gang,history,redownload}:ProductionDialogProps){
      ...
      :production&&<>
        <div className="production-layout">
          ...
          <div className="production-report">
            ...
            {production.qaProofs?.[0]?<div className="vector-qa-proof"><div><strong>Vector similarity overlay</strong><small><i className="match"/> matched <i className="missing"/> missing <i className="extra"/> extra</small></div><img src={production.qaProofs[0]} alt="Vector QA overlay: green matched pixels, red missing pixels, blue extra pixels"/></div>:null}
            {production.ocrSuggestions?.map(suggestion=><div className="ocr-rebuild" key={suggestion.elementId}><strong>Recognized raster text</strong><p>“{suggestion.text}” · {suggestion.confidence.toFixed(1)}%</p><small>Best measured studio-font match: {suggestion.fontFamily} {suggestion.fontWeight}. The original image remains unchanged until you confirm.</small><button type="button" disabled={!suggestion.recommended} onClick={()=>rebuildRecognizedText(suggestion)}>{suggestion.recommended?"Rebuild as editable text":"OCR or font match needs confirmation"}</button></div>)}
...
function MethodSettings({method,iccCombo,setIccCombo,iccCombinations,mirrorVinyl,setMirrorVinyl,vinylMaterialClass,setVinylMaterialClass,advancedVectorize,setAdvancedVectorize,dtfTrapPreset,setDtfTrapPreset,dtfUnderbaseSpread,setDtfUnderbaseSpread,screenLpi,setScreenLpi,screenMode,setScreenMode,screenTrapPreset,setScreenTrapPreset,embroideryDefaults,setEmbroideryDefaults,gang}:MethodSettingsProps){
  ...
  return <div className="method-settings">
    ...
    {key.includes("vinyl")&&<>
      ...
      <label className="check"><input type="checkbox" checked={advancedVectorize} onChange={e=>setAdvancedVectorize(e.target.checked)}/> Advanced vectorization (AI cleanup + smart trace)</label>
      <p className="hint">Off: direct Potrace traces the complete uploaded image bounds with no background removal, cropping, edge repair or supersampling. On: content-aware cleanup, edge repair and AI preparation are added for clean logos. Direct is best when any part of the image was being lost.</p>
    </>}
    {key.includes("screen")&&<>
      ...
      <label className="check"><input type="checkbox" checked={advancedVectorize} onChange={e=>setAdvancedVectorize(e.target.checked)}/> Advanced vectorization (AI cleanup + smart trace)</label>
      <p className="hint">Off uses direct, bounds-preserving colour separation. On adds background isolation, edge repair and a screen-specific trace. Continuous-tone artwork stays raster for the screen pack and halftone processors.</p>
    </>}
    {key==="embroidery"&&<>
      ...
      <label className="check"><input type="checkbox" checked={advancedVectorize} onChange={e=>setAdvancedVectorize(e.target.checked)}/> Advanced vectorization (AI cleanup + smart trace)</label>
      <p className="hint" style={{marginTop:0}}>Off traces the full upload directly. On uses automatic lettering/logo detection, repair and stitch-stable settings before digitizing. Editable text stays glyph-traced.</p>
