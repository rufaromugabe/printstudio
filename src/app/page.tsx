...
export default function Home() {
  ...
  const insertAsset=async(asset:Asset)=>{let url=asset.url;if(!url)url=(await api.assetURL(asset.id)).url;const ratio=asset.width/asset.height;const w=ratio>=1?180:180*ratio;const h=ratio>=1?180/ratio:180;const el:Element={id:crypto.randomUUID(),type:"image",value:url,assetId:asset.id,sourceWidth:asset.width,sourceHeight:asset.height,x:Math.max(0,(currentView.canvasWidth-w)/2),y:Math.max(0,(currentView.canvasHeight-h)/2),w,h,rotation:0,color:"",fontSize:0};commit(d=>({...d,elements:{...d.elements,[d.side]:[...(d.elements[d.side]??[]),el]}}));setSelected(el.id);setUploadState(`${asset.width} × ${asset.height}px added`);void suggestBackgroundCleanup(el.id,url)};
