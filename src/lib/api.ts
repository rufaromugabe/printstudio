...
export type EmbroideryRequest={name:string;fabricClass?:EmbroideryFabricClass;regions:EmbroideryRegion[];machine:{id:string;name:string;hoopWidthMm:number;hoopHeightMm:number;maxStitches:number;maxColors:number;minStitchMm:number;maxStitchMm:number;maxJumpMm:number};printWidthMm?:number;printHeightMm?:number};
