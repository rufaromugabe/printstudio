...
export const api = {
  ...
  productionDTFPack:(artwork:Blob,options:{name:string;widthMm:number;heightMm:number;dpi?:number;spread?:number;threshold?:number;trapPreset?:string})=>{const query=new URLSearchParams({name:options.name,widthMm:String(options.widthMm),heightMm:String(options.heightMm),dpi:String(options.dpi??300),spread:String(options.spread??2),threshold:String(options.threshold??1)});if(options.trapPreset)query.set("trapPreset",options.trapPreset);return requestBlob(`/v1/production/dtf/pack?${query}`,artwork)},
  ...
  productionScreenPack:(artwork:Blob,options:{name:string;widthMm:number;heightMm:number;dpi?:number;lpi?:number;gamma?:number;underbaseChoke?:number;screening?:"am"|"fm";trapPreset?:string;requireIcc?:boolean;sourceProfile?:string;destinationProfile?:string})=>{const query=new URLSearchParams({name:options.name,widthMm:String(options.widthMm),heightMm:String(options.heightMm),dpi:String(options.dpi??300),lpi:String(options.lpi??45),gamma:String(options.gamma??1),underbaseChoke:String(options.underbaseChoke??-2),screening:options.screening??"am"});if(options.trapPreset)query.set("trapPreset",options.trapPreset);if(options.requireIcc)query.set("requireIcc","true");if(options.sourceProfile)query.set("sourceProfile",options.sourceProfile);if(options.destinationProfile)query.set("destinationProfile",options.destinationProfile);return requestBlob(`/v1/production/screen/pack?${query}`,artwork)},
};
